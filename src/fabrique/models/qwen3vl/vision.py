...
class VisionAttention(nnx.Module):
    ...
    def _create_attention_mask(self, cu_seqlens: jax.Array, seq_len: int) -> jax.Array:
    ...
    def __call__(
        ...
        cos_f = cos[:, : self.head_dim][:, None, :]  # (seq_len, 1, head_dim) float32
        sin_f = sin[:, : self.head_dim][:, None, :]  # (seq_len, 1, head_dim) float32
