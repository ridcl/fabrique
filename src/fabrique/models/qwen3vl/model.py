...
def get_rope_index(
    ...
    image_grid_np = np.array(image_grid_thw) if image_grid_thw is not None else None
    video_grid_np = np.array(video_grid_thw) if video_grid_thw is not None else None
    ...
    mrope_deltas_jax = jnp.array(mrope_position_deltas, dtype=jnp.int32)[:, np.newaxis]
...
class Qwen3VL(BackendMappingMixin, nnx.Module):
    ...
    def __call__(
        ...
        if self.config.vision_config and pixel_values is not None:
            ...
            vision_embeds = vision_embeds.cast(self.config.param_dtype).with_batch_dim(
                bsz
            )
    ...
    def get_model_input(self):
        ...
        return {
            ...
            "cache": None,
            "padding_mask": jnp.ones((dummy_batch_size, dummy_seq_len), dtype=jnp.bool),
        }
